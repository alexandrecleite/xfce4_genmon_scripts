<H1>Scripts para monitorar o sistema no painel via xfce4-genmon</H1>
<p align="center">
<BR>
<img width="633" height="20" alt="genmon06" src="https://github.com/user-attachments/assets/b4ab8337-9fcf-43d0-8924-9eb4115c0474" />
</p>
<BR><BR>
Scripts para monitoramento do sistema no painel do XFCE4 através do Monitor Genérico (xfce4-genmon).
Os tempos sugeridos de atualização estão nos comentários de cada script. São separados para poder escolher a ordem que quiser.
<BR><BR>
Se parar com o mouse em cima de cada um, aparece mais informações de cada ítem. (Tooltips)
<BR>
Se clicar com o botão esquerdo ele abre algum programa relativo ao item. 
<BR><BR>
Abaixo alguns exemplos das tooltips:
<BR><BR>
<p align="center">
CPU<BR>  
<img width="466" height="296" alt="CPU" src="https://github.com/user-attachments/assets/ebe4efcb-4865-4ae5-a9ba-4d0e66d426b6" />
<BR><BR>  
Temperatura CPU:<BR>
<img width="434" height="231" alt="Temp" src="https://github.com/user-attachments/assets/0ad5841f-15d4-4dbb-909d-6c99df2f753c" />
<BR><BR>
Memória:<BR>
<img width="248" height="239" alt="Mem" src="https://github.com/user-attachments/assets/835ce779-3d20-4da2-89f6-5d7f2f4ef773" />
<BR><BR>
Disco:<BR>
<img width="579" height="276" alt="HD" src="https://github.com/user-attachments/assets/8828f050-f1d5-41b0-aa10-93ef509e01a7" />
<BR><BR>
Clima<BR> 
<img width="213" height="205" alt="Clima" src="https://github.com/user-attachments/assets/92c73632-3813-4820-b7bb-2e174eb790f9" />
<BR><BR> 
Updates:<BR>
<img width="581" height="99" alt="Updates" src="https://github.com/user-attachments/assets/98af6b24-560e-4c75-ad73-84e600b2c45c" />
<BR><BR>
Rede:<BR>
<img width="377" height="158" alt="Rede" src="https://github.com/user-attachments/assets/4f0c8786-10a7-4374-a0f8-ce96e52a40c7" />
</p>
<BR><BR> <BR><BR> 
<H3>Tutorial de instalação no meu Blog:</H3>
https://acleite.wordpress.com/2026/04/07/personalize-o-painel-do-xfce-com-informacoes-do-sistema/
